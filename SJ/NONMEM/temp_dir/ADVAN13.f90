...
!               JAN/2021 - nonmem 7.5.1 #2, PC memory allocation correction (PCMORE)
!
!----------------------------- ADVAN13.f90 -------------------------------------------
!
...
!
      ...
      SUBROUTINE ADVAN(ICALL)
!      
      USE PRSIZES,      ONLY: ISIZE,DPSIZE,PE,PC,MAXFCN,PG,PIR,PCMORE
...
! INTEGER
      ...
      USE PRCOM_INT,    ONLY: ADVID,BETA,CALCTR,IATT,IDNO,IH,IMAX,ISPEC,LINK,LOGUNT,    &
                              MAXCAL,MCOMP,MITER,NBP,NCM1,NPETAS2=>NPETAS,NRD,SV,NY
      USE PRCM_INT,     ONLY: INRD,MFLAG8,LST,LEND,SLEND,SLST,SPW,SLOOP,IANRD,ITOTL,MAPINV, &
                              ...
                              LST,LEND,SLEND,SLST,SLOOP,MFLAGDDE,LNCM2
      USE PRLS01_INT,  ONLY:  ITASK
      USE PR_BAYES_INT, ONLY: METH
! REAL
...
! Local Variables
!
      INTEGER(KIND=ISIZE), PARAMETER :: PM=PC-1,PPW=2*PE*PM+PM+PCMORE*((PC-1)*(PE+1)*PE)/2, &
                                        P99=PC*PE+PC-PE-1+PCMORE*((PC-1)*(PE+1)*PE)/2, P11=PE+1
      INTEGER(KIND=ISIZE) :: I,I2FLAG,ICOMP,IER,IND,IOUT,J,K,KLOOP,L,METHOD,NPR,ILOOP
!      
      REAL(KIND=DPSIZE)   :: DADT,DADT2,H,OLDA,OLDAE2,OLDAET,TL,X,XEND,Y

      REAL(KIND=DPSIZE)   :: RWORK(22+9*P99+P99*P99)

      INTEGER(KIND=ISIZE) :: ISTATE,IWORK(20+P99),LIW,LRW,MF,MFLAGA,MFLAGX,&
                             MFLAGY,MFLAGZ,IREDO
...
!        ITASK=1
        RWORK(1)=STOP_TIME
        IF(RWORK(1)==-1.0D+300) RWORK(1)=XEND
        IREDO=1
        IF(ITASK==2.OR.ITASK==3) IREDO=0
  340   CONTINUE   
        ILOOP=-1    ! ADD-ON DDE RJB, NM75 JUN 2020
        DO WHILE(ILOOP/=0) ! DDE ADD-ON, RJB nm75b6, June 2020
          CALL DDE_WRAP(RES2,ILOOP,NY,Y,DADT,X,XEND,ITASK,ISTATE,DADT,RWORK,DADT,DADT,P99,P99)
          IF(ILOOP==2) EXIT
          CALL LSODA(RES2,NY,Y,X,XEND,ITOL,RTOL,ATOL,ITASK,ISTATE,IOPT,RWORK,LRW,IWORK,LIW,JAC,MF)
        ENDDO
        IF(X<XEND.AND.IREDO==0) THEN
          ISTATE=2
!          ITASK=1
          RWORK(1)=STOP_TIME
          IF(RWORK(1)==-1.0D+300) RWORK(1)=XEND
...
!
      END SUBROUTINE RES2
      SUBROUTINE TSTOP_SETUP(ITASK,RWORK,XEND)
      USE PRSIZES,      ONLY: ISIZE,DPSIZE
      IMPLICIT NONE
      INTEGER(KIND=ISIZE)     :: ITASK
      REAL(KIND=DPSIZE)     :: RWORK(*),XEND
      ITASK=5
      RWORK(1)=XEND
      RETURN
      END SUBROUTINE TSTOP_SETUP
