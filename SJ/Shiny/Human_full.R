  ...
  dosing <- eventReactive(input$simulate, {
    ...
    rxsolve <- eventReactive(input$simulate, {
      rxSolve(model, c(LNDOSE = log(as.double(input$dose)), t.KTR3 = 0.0000060*as.double(input$ktr3),
                       t.KTR = 0.0001100*as.double(input$ktr), t.CL39 = 0.00000025*as.double(input$cl39),
                       t.KTR5 = 0.0111000*as.double(input$ktr5), t.KTR2 = 0.0008550 *as.double(input$ktr2),
                       t.KTR4 = 0.0023500 *as.double(input$ktr4), t.KTR6 = 0.0008470*as.double(input$ktr6)
                       ), dosing(), nSub = 1)
    })
    ...
    output$simlationplot1 <- renderPlotly({
      ...
      plot <- ggplot() +
        ...
        )  + scale_x_continuous(breaks = seq(0, max(4,as.double(input$num)*as.double(input$interval)),12)) + 
        geom_hline(yintercept = input$abline1, color = "yellow3") + 
        geom_hline(yintercept = input$abline2, color = "darkorange2")
        plot <- ggplotly(plot)
    })
    ...
    output$simlationplot3 <- renderPlotly({
      ...
      plot <- ggplot() +
        ...
        labs(x = "Time (Month)", y = "SB15 Concentration (ng/mL)", color = NULL, title="Retina") + 
        theme(axis.text.x = element_text(vjust = 0.5, size = 12),
              axis.text.y = element_text(vjust = 0.5, size = 12),
              axis.title.y = element_text(size = 14, margin = margin(t = 0, r = 20, b = 0, l = 0)),
              axis.title.x = element_text(size = 14, margin = margin(t = 10, r = 0, b = 0, l = 0)),
    ...
    output$simlationplot5 <- renderPlotly({
      ...
      plot <- ggplot() +
        ...
        labs(x = "Time (Month)", y = "SB15 Concentration (ng/mL)", color = NULL, title="Optic nerve") + 
        theme(axis.text.x = element_text(vjust = 0.5, size = 12),
              axis.text.y = element_text(vjust = 0.5, size = 12),
              axis.title.y = element_text(size = 14, margin = margin(t = 0, r = 20, b = 0, l = 0)),
              axis.title.x = element_text(size = 14, margin = margin(t = 10, r = 0, b = 0, l = 0)),
    ...
    output$simlationplot6 <- renderPlotly({
      ...
      plot <- ggplot() +
        ...
        labs(x = "Time (Month)", y = "SB15 Concentration (ng/mL)", color = NULL, title="IRIS") + 
        theme(axis.text.x = element_text(vjust = 0.5, size = 12),
              axis.text.y = element_text(vjust = 0.5, size = 12),
              axis.title.y = element_text(size = 14, margin = margin(t = 0, r = 20, b = 0, l = 0)),
              axis.title.x = element_text(size = 14, margin = margin(t = 10, r = 0, b = 0, l = 0)),
    ...
    output$simlationplotlog1 <- renderPlotly({
      ...
      plot <- ggplot() +
        ...
        geom_hline(yintercept = input$abline1, color = "yellow3") + 
        geom_hline(yintercept = input$abline2, color = "darkorange2") + scale_y_log10()
      plot <- ggplotly(plot)
    })
